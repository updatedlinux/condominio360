...
class AdminController {
     ...
     * Tasas BCV vigentes en BD (misma referencia que ven los propietarios).
     */
    static async getBcvSettings(req, res) {
        try {
            ...
            res.status(500).json({ success: false, error: 'Error al obtener tasas BCV' });
        }
    }
     ...
     * body: { visitsEnabled, deliveriesEnabled, vehicleAccessEnabled, commonAreasEnabled }
     */
    static async updateTenantPortalFeatures(req, res) {
        try {
            const { id } = req.params;
            const { visitsEnabled, deliveriesEnabled, vehicleAccessEnabled, commonAreasEnabled } = req.body || {};
            ...
            await AdminController.logAudit(
                ...
                'TENANT',
                id,
                `Módulos portal: visitas=${visitsEnabled ? 1 : 0}, deliveries=${deliveriesEnabled ? 1 : 0}, vehicular=${vehicleAccessEnabled ? 1 : 0}, áreas=${commonAreasEnabled ? 1 : 0}`,
                id
            );
            res.json({ success: true, data });
        } catch (error) {
