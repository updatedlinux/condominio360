...
const OwnerInvoiceDetailService = require('../services/OwnerInvoiceDetailService');
...
class OwnerBillingController {
    ...
    static async getInvoiceById(req, res) {
        try {
            const { id } = req.params;
            const ctx = await OwnerInvoiceDetailService.prepareForOwner(
                id, req.user.tenantId, req.user.userId
            );
            const { invoice, displayStatus } = ctx;
    ...
    /**
     * GET /api/owner/billing/invoices/:id/pdf
     * Descargar comprobante PDF del recibo (pagado, pendiente o pago parcial).
     */
    static async getInvoicePdf(req, res) {
        try {
            ...
            BillingInvoicePdfService.streamOwnerReceipt(res, ctx);
        } catch (error) {
            if (error.status === 403) return res.status(403).json({ error: error.message });
            if (error.status === 404) return res.status(404).json({ error: error.message });
